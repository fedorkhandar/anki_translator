    ...
    async def send(self):
        params = {
            "note": {
                ...
                "fields": {"Back": self.back, "Front": self.front},
                "modelName": "Basic",
            }
        }
...
async def create_note(data: Dict[str, Any]):
    ...
    a = PubNote(data["deck_name"], data["front"], data["back"])
    await a.send()
...
class AnkiNote:
    ...
    def from_json(cls, data: JsonType) -> Optional[Self]:
        ...
        def extract_word(
            data: JsonType
        ) -> Tuple[str, str, str, List[AnkiMeaning]]:
            deck = data.get("deck_name", "OOP")
            word = data.get("word", "")
            ...
            return deck, word, phonetic, meanings
    ...
    def htmled_front(self):
        ...
        for meaning in self.meanings:
            ...
            for u in meaning.definitions:
                ...
                result += (
                    f'{j}. <span style="color:{pos_color}; font-size: 50%;">'
                    f'{meaning.part_of_speech}</span> '
                    f'{clear_definition(u.definition, self.word)}<br>'
                )
                j += 1
    ...
    def htmled_back(self):
        # TODO: templates representation and separate storing
        result = (
            f'<div style="margin:10px;"><b>{self.word}</b><br>{self.phonetic}</div>'
        )
        j = 0
        for meaning in self.meanings:
            result += '<div style="margin:10px;">'
            for u in meaning.definitions:
                ...
                result += (
                    f'{j}. <span style="color:{pos_color};font-size: 50%;">'
                    f'{meaning.part_of_speech}</span>'
                    f'{highlight(u.definition, self.word)}<br>'
                )
                j += 1
                if u.example:
                    result += settings.EXAMPLE_STR.format(
                        highlight(u.example, self.word)
                    )
                if u.synonyms:
                    result += settings.SYNONYM_STR.format(u.synonyms)
                if u.antonyms:
                    result += settings.ANTONYM_STR.format(u.antonyms)
            result += "</div>"
        return result
...
class Card(Base):
    ...
    async def get_parents(cls, session: aiosqlite.Connection) -> List[str]:
        ...
        FROM word_phrase WHERE to_card = 0 
        ...
        for r in rows:
            result.extend([
                ...
                for w in orjson.loads(r[1]) # noqa: E1101
            ])
        return result

    @classmethod
    async def from_parent(
        cls, parent: str, session: aiosqlite.Connection
    ) -> List[Self]:
        word_phrase_id_fk = parent[0]
        body = orjson.dumps(parent[1].to_dict()) # pylint: disable=maybe-no-member
        return cls(body=body, word_phrase_id_fk=word_phrase_id_fk)
    ...
    async def save(cls, session: aiosqlite.Connection, sources: List[Self]) -> None:
        ...
        VALUES (?, ?)
        ON CONFLICT(body) DO 
        UPDATE SET word_phrase_id_fk = excluded.word_phrase_id_fk
        RETURNING id;
        """
        tasks = []
        processed_ids = []
        for s in sources:
            await db.insert_query_returning(
                session, query1, (s.body, s.word_phrase_id_fk)
            )
            processed_ids.append(s.word_phrase_id_fk)
            tasks.append(
                asyncio.create_task(
                    create_note(orjson.loads(s.body)) # pylint: disable=maybe-no-member
                )
            ) 
