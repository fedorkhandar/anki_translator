"""
Class CleanedSource -- automatically cleaned 
and formatted from srt to text lines
Utilities for cleaning and formatting srt files
...
def clean_srt(body):
    """cleans SRT-file from extra symbols and comments"""
    lines = [line for line in body.split("\n") if line != ""]

    res_lines = []

    for line in lines:
        line = clean_extras(line)
        if len(line) > 0:
...
async def line_exists(session: aiosqlite.Connection, line: str) -> bool:
    """check if line exists in db"""
    res = await db.insert_once(session, 'line', 'body', line)
    if res:
        return line
    return ""


async def archive_lines(
    ...
    tasks = [asyncio.create_task(line_exists(session, line)) for line in lines]
...
class CleanedSource(Base):
    """
    Cleaned Source -- automatically cleaned and formatted from srt to text lines
    """
    ...
    async def from_parent(
        cls, parent: Source, session: aiosqlite.Connection = None
    ) -> Self:
        """create cleaned source from source"""
        cleaned_body = clean_srt(parent.body)
        cleaned_body = await archive_lines(session, cleaned_body)
        return cls(body=cleaned_body, hash=calc_hash(cleaned_body), parent_id=parent.id)
    ...
    async def get_parents(cls, session: aiosqlite.Connection) -> List[Source]:
        ...
        query = """
        WITH cte AS (
            SELECT DISTINCT source_id_fk
            FROM source_m2m_cleaned_source
        )
        SELECT * FROM source WHERE id NOT IN
        (SELECT source_id_fk FROM cte);
        """
    ...
    async def save(cls, session: aiosqlite.Connection, sources: List[Self]) -> None:
        ...
        for s in sources:
            returning_id = await db.insert_query_returning(
                session, query1, (s.body, s.hash)
            )
            sp.append((s.parent_id, returning_id))

        query2 = """
        INSERT INTO source_m2m_cleaned_source
        (source_id_fk, cleaned_source_id_fk)
        VALUES (?, ?);
        """

        for p in sp:
            await db.insert_query(session, query2, (p[0], p[1]))
